...
#  filter(health_estbl_code %in% n200_hosp) %>%
  ggplot(aes(x= birth_year, y = Cesarean)) +
    ...
    labs(x = "Birth year", y = "Proportion", color = "Delivery type") +
#    facet_wrap(~health_estbl_code + state_name)
    facet_trelliscope(~health_estbl_code + state_name + birth_muni_code, as_plotly = TRUE, width = 650, path = "trelliscope", nrow = 3, ncol = 3)
