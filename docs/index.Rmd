...
library(forcats)
library(geofacet)
invisible(try(file.copy("../_fns.R", ".", overwrite = TRUE), silent = TRUE))
source("_fns.R")
...
ggplot(brthwt_year_state, aes(birth_year, pct_low)) +
  ...
  facet_geo(~ m_state_code, grid = "br_states_grid2", label = "name") +
  theme(strip.text.x = element_text(margin = margin(0.1, 0, 0.1, 0, "cm"), size = 7)) +
  labs(x = "Birth Year", y = "Percentage of Children Born with Low Birth Weight")
```
...
ggplot(brthwt_muni_region, aes(rank, mean_bwt, color = region_code)) +
  ...
  labs(y = "Mean Birth Weight", x = "Municipality Rank (within region)")
```
...
ggplot(filter(brthwt_inc, n >= 10 & !is.na(mean_bwt)), aes(mean_inc, mean_bwt)) +
  ...
  theme_bw() +
  labs(
    # title = "Mean birth weight vs. mean income for each municipality by census year",
...
ggplot(brthwt_inc_ga, aes(mean_inc, mean_bwt)) +
  ...
  theme_bw() +
  labs(
    # title = "Mean birth weight vs. mean income for each municipality by gestational age at birth and census year",
    x = "Mean monthly income (R$)",
    y = "Mean birth weight (g)")
```
...
```{r echo=FALSE, warning=FALSE, message=FALSE, fig.height=7, fig.width=8.5}
